...
const ActivityList = ({activities, dispatch} : ActivityListProps) => {
   ...
   const isEmpty = useMemo(() => activities.length === 0, [activities])
    ...
    <>
        ...
        {
        isEmpty ? <p className="text-center my-5">
            No activities yet
        </p> :

        activities.map(activity => (
            <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between">
                <div className="space-y-2 relative mx-auto lg:mx-0" >
                    <p className={`aboslute -top-8 -left-8 px-10 py-2 rounded-md text-center text-white uppercase font-bold ${activity.category === 1 ? 'bg-lime-500' : 'bg-orange-500'}`}>
